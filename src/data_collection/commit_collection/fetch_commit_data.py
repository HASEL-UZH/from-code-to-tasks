                ...
                try:
                    if None not in commit.modified_files:
                        for modified_file in commit.modified_files:
                            ...
                            source_code_before_path = os.path.join(
                                folder_name, f"{file_name}_before"
                            )
                            if source_code_before is not None:
                                with open(
                                    source_code_before_path, "w"
                                ) as source_code_before_file:
                                    source_code_before_file.write(source_code_before)

                            source_code_after_path = os.path.join(
                                folder_name, f"{file_name}_after"
                            )
                            if source_code_after is not None:
                                with open(
                                    source_code_after_path, "w"
                                ) as source_code_after_file:
                                    source_code_after_file.write(source_code_after)

                            commit_diff_path = os.path.join(
                                folder_name, f"{file_name}.diff"
                            )
                            with open(commit_diff_path, "w") as commit_diff_file:
                                commit_diff_file.write(commit_diff)

                            commit_info_path = os.path.join(
                                folder_name, "commit_info.json"
                            )
